...
int main(int argc, char const *argv[])
{
    ...
    WORD modo_cor = (IDYES == MessageBox(window_handler, "Deseja usar o modo escuro?", "Escolha o modo:", MB_YESNO)) ? 0x0F : 0xF0;

    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), modo_cor);
    ...
    bool aleatorio = false, realizar_jogo = false;
    ...
    // Exibe o campeão
    limpar_terminal();
    imprimir_ganhador(teams_fase_grupo[0].nome, GetStdHandle(STD_OUTPUT_HANDLE), modo_cor);
    printf(MSG_ENTER_CONTINUAR);
    limpar_buffer();
    getchar();
    ...
    // Fim do programa
    limpar_terminal();
    MessageBox(window_handler, "Obrigado por usar o nosso sistema.\nClique OK para sair.", "Fim!", MB_OK);
